...
class Model_Shippingmethod extends Model_Core_Table
{
    ...
    {
        $sql = "SELECT * FROM `eav_attribute` WHERE `entity_type_id` = 9 ";
    ...
    {
        if ($this->getId()) {
            $query = "SELECT `value` FROM `shipping_method_{$attribute->backend_type}` WHERE `entity_id` = '{$this->getId()}' AND `attribute_id` = '{$attribute->getId()}'";
